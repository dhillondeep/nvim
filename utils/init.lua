...
deepvim.on_file_open = { "BufRead", "BufWinEnter", "BufNewFile" }
...
function deepvim.bool2str(bool) return bool and "on" or "off" end

function deepvim.ui_notify(str)
  if vim.g.ui_notifications_enabled then deepvim.notify(str) end
end
...
function deepvim.notify(msg, type, opts)
  vim.schedule(function() vim.notify(msg, type, deepvim.default_tbl(opts, { title = "DeepVim" })) end)
end
...
function deepvim.default_tbl(opts, default)
  opts = opts or {}
